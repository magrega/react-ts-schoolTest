...
const QuestionCard = ({ type }: { type: string }) => {
  ...
  const lSanswersBatchNum = JSON.parse(
    localStorage.getItem("answersBatchNum")!
  );
  ...
  const [answersBatchNum, setAnswersBatchNum] = useState<number>(
    lSanswersBatchNum || 0
  );
  ...
  const renderQuestionType = (type: string) => {
    switch (type) {
      ...
      case "short-written":
        return (
          <WrittenQuestions text={writtenAnswer} setText={setWrittenAnswer} />
        );
      case "long-written":
        return (
          <WrittenQuestions
            text={writtenAnswer}
            setText={setWrittenAnswer}
            isLong
          />
        );
      default:
  ...
  const handleFormChange = (e: ChangeEvent<HTMLFormElement>) => {
    ...
    if (inputType === "checkbox") {
      ...
      checkboxes.forEach((input) =>
        isSomeChecked ? (input.required = false) : (input.required = true)
      );

      if (userAnswer.includes(answerValue)) {
        setUserAnswer((prev) =>
          prev.filter((answer) => answer !== answerValue)
        );
      } else {
        setUserAnswer((prev) => [...prev, answerValue]);
      }
    ...
    <div className={styles["quiz-container"]}>
      ...
      <div className={styles["quiz-progress-bar"]}>
        {questions.map((_, index) => {
          const isCurrent =
            index === questionNum ? `${styles["current-question"]}` : "";
          const isAnswered = index < questionNum ? `${styles.answered}` : "";
          return (
            <span
              key={index}
              className={`${styles.bar} ${isCurrent} ${isAnswered}`}
            />
          );
        })}
      </div>
      <div className={styles.card}>
        <p
          className={styles["card-question"]}
        >{`${questions[questionNum]}?`}</p>
        <form
          onChange={handleFormChange}
          onSubmit={handleNextQuestion}
          id="quiz"
          className={styles["card-radio-container"]}
        >
          {renderQuestionType(type)}
          <button className={styles["quiz-submit-btn"]} type="submit">
